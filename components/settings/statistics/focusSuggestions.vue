...
const efficiencySuggestions = computed(() => {
  ...
  if (focusEfficiency.value < 0.5) {
    suggestions.push(t('settings.values.statistics.suggestions.lowEfficiency'))
  } else if (focusEfficiency.value < 0.8) {
    suggestions.push(t('settings.values.statistics.suggestions.mediumEfficiency'))
  } else {
    suggestions.push(t('settings.values.statistics.suggestions.highEfficiency'))
  }
...
const interruptionSuggestions = computed(() => {
  ...
  if (averageInterruptions.value > 2) {
    suggestions.push(t('settings.values.statistics.suggestions.highInterruptions'))
  } else if (averageInterruptions.value > 0.5) {
    suggestions.push(t('settings.values.statistics.suggestions.mediumInterruptions'))
  } else {
    suggestions.push(t('settings.values.statistics.suggestions.lowInterruptions'))
  }
...
const timingSuggestions = computed(() => {
  ...
  if (bestFocusTime.value !== null) {
    suggestions.push(t('settings.values.statistics.suggestions.bestTimeOfDay', { time: formatHour(bestFocusTime.value) }))
  }
  
  if (bestFocusDay.value !== null) {
    suggestions.push(t('settings.values.statistics.suggestions.bestDayOfWeek', { day: formatDay(bestFocusDay.value) }))
  }
...
<template>
  <div class="focus-suggestions">
    <h3 class="text-lg font-medium mb-2">{{ $t('settings.values.statistics.suggestions.title') }}</h3>
    ...
    <div class="suggestions-container bg-surface-light dark:bg-surface-dark p-4 rounded-lg">
      ...
      <div v-else class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
        ...
        <p>{{ $t('settings.values.statistics.suggestions.notEnoughData') }}</p>
      </div>
    </div>
  </div>
</template>
